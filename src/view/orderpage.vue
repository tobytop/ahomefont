<template scoped>
  <div style="background-color: #f7f8fa;height:100vh">
    ...
    <van-cell v-else is-link class="selfaddress" @click="showAddress">
      ...
      <template #right-icon>
        <van-icon name="arrow" class="showaddress" />
      </template>
    </van-cell>
    <van-card
      num="2"
    ...
    <div style="margin-top:10px">
      <van-cell title="贴心服务(选填)" is-link @click="showService">
        ...
        <template #default>
          <div class="van-info">
            {{ result.length == 0 ? "" : result.length.toString() }}
          </div>
        </template>
      </van-cell>
    </div>
